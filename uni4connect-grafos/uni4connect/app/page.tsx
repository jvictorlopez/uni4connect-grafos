...
function GraphRenderer({ data, onNodeClick, currentUserId }: GraphRendererProps) {
  ...
  useEffect(() => {
    ...
    const node = svg
      ...
      .on("click", (event, d) => {
        console.log("Clicou no nó:", d);
        if (d.id !== currentUserId) onNodeClick(d)
      })
      .call(
        d3
  ...
  }, [data, onNodeClick, currentUserId])
    ...
    return (
      <div className="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center bg-gray-50 min-h-[400px] flex flex-col items-center justify-center">
        ...
        <div className="text-xs text-gray-400 bg-white px-3 py-1 rounded border">
          🔧 Integração D3.js: implementada
        </div>
      </div>
    )
  }

  return <div ref={ref} className="rounded border bg-white p-2" style={{ minHeight: "400px" }} />
}
...
function ModalAluno({ node, onClose }: ModalAlunoProps) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="relative bg-white p-6 rounded-lg shadow-xl max-w-md w-full">
        ...
        <h2 className="text-2xl font-bold text-gray-800 mb-4">{node.label}</h2>

        <ul className="space-y-2 text-sm text-gray-700">
          <li>
            <strong>Curso:</strong> {node.group}
          </li>
          <li>
            <strong>Comunidade:</strong> {node.community}
          </li>
          <li>
            <strong>Centralidade:</strong> {Math.round(node.centrality * 100)}%
          </li>
        </ul>
  ...
  {selectedNode && (
  <ModalAluno node={selectedNode} onClose={() => setSelectedNode(null)} />
)}
            ...
            <Card className="shadow-lg border-0 bg-white/80 backdrop-blur h-full">
              ...
              <CardContent className="p-6">
                ...
                <GraphRenderer
                  ...
                  currentUserId={graphData?.nodes.find((n) => n.id > 100)?.id || 0}
                />
